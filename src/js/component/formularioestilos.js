import React from "react";

function Formularioestilos(props) {
    return (
        <form>
            <div className="row formulario-estilos-body">

                <div className="column">
            
                    <img src="look1.jpg" style="width 100%"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look1" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 1</span>
                    </label>

                    <img src="look2.jpg" style="width 100%"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look2" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 2</span>
                    </label>

                    <img src="look3.jpg" style="width 100%"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look3" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 3</span>
                    </label>

                    <img src="look4.jpg" style="width 100%"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look4" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 4</span>
                    </label>

                    <img src="look5.jpg" style="width 100%"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look5" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 5</span>
                    </label>

                    <img src="look6.jpg" style="width 100%"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look6" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 6</span>
                    </label>

                    <img src="look7.jpg" style="width 100%"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look7" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 7</span>
                    </label>
                    
                    <img src="look8.jpg" style="width 100%"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look8" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 8</span>
                    </label>

                    <img src="look9.jpg" style="width 100%"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look9" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 9</span>
                    </label>

                    <img src="look10.jpg" style="width 100%"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look10" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 10</span>
                    </label>

                </div>

                <div className="column">

                    <img src="look10.jpg"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look11" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 11</span>
                    </label>

                    <img src="look11.jpg"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look12" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 12</span>
                    </label>

                    <img src="look12.jpg"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look13" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 13</span>
                    </label>

                    <img src="look13.jpg"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look14" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 14</span>
                    </label>

                    <img src="look14.jpg"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look15" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 15</span>
                    </label>

                    <img src="look15.jpg"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look16" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 16</span>
                    </label>

                    <img src="look16.jpg"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look17" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 17</span>
                    </label>

                    <img src="look17.jpg"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look18" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 18</span>
                    </label>

                    <img src="look18.jpg"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look19" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 19</span>
                    </label>

                    <img src="look19.jpg"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look20" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 20</span>
                    </label>

                </div>

                <div className="column">

                    <img src="look21.jpg"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look21" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 21</span>
                    </label>
    
                    <img src="look22.jpg"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look22" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 22</span>
                    </label>

                    <img src="look23.jpg"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look23" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 23</span>
                    </label>

                    <img src="look24.jpg"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look24" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 24</span>
                    </label>

                    <img src="look25.jpg"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look25" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 25</span>
                    </label>

                    <img src="look26.jpg"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look26" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 26</span>
                    </label>

                    <img src="look27.jpg"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look27" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 27</span>
                    </label>

                    <img src="look28.jpg"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look28" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 28</span>
                    </label>

                    <img src="look29.jpg"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look29" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 29</span>
                    </label>

                    <img src="look30.jpg"></img>
                    <label className="checkbox-label">
                    <input type="checkbox" name="look30" />
                    <i class="form-checkbox-button"></i>
                    <span>Look 30</span>
                    </label>

                </div>

                <input type="button" onclick="funcionestilos()" value="Estilos preferidos"/>
            </div>
        </form>
        )
    }
            
    function funcionestilos() {
                var lookchoice = document.forms[0];
                var i;
                for (i = 0; i < 6; i++) {
                    if (lookchoice[i].checked) {
                            Estilos = Estilos + lookchoice[i].value;
                        }
                    }
                }
            

export default Formularioestilos;